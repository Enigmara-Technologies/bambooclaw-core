// Error boundary is in Script 1 (canary) to catch parse errors in this script block.
    ...
    async function callLLM(userMessage) {
        ...
        if (autonomy === "observe") {
            ...
            systemPrompt += "7. ENCODING RULE: NEVER output raw Unicode characters (like German umlauts ä, ö, ü, or the Euro symbol €) inside Python scripts. ALWAYS use standard ASCII equivalents (e.g. 'ae', 'oe', 'ue', 'EUR') to prevent Windows encoding crashes.\n\n";
        }
    ...
    function showConnectionModal(slug) {
        ...
        overlay.innerHTML = '<div class="conn-modal">' +
            ...
            '<button class="btn-oauth" id="conn-btn-oauth">🔗 Connect via OAuth (opens browser)</button>' +
    ...
    function renderBuiltinSkills() {
        ...
        builtinSkills.forEach(function(s) {
            ...
            html += '<span style="font-size:0.65rem;color:var(--text-dim);">● Built-in</span>';
    ...
    function renderComposioToolkits() {
        ...
        filtered.forEach(function(tk) {
            ...
            if (imgSrc) {
                html += '<div class="skill-icon"><img src="' + escapeHtml(imgSrc) + '" data-slug="' + escapeHtml(tk.slug) + '" onerror="this.outerHTML=\'<span class=skill-icon>🧩</span>\'" /></div>';
            } else {
            ...
            html += '<div class="skill-meta">';
            if (toolCount > 0) {
                html += '<span style="font-size:0.65rem;color:var(--accent);">● ' + toolCount + ' tools</span>';
            }
            if (triggerCount > 0) {
                html += '<span style="font-size:0.65rem;color:var(--warning);">⚡ ' + triggerCount + ' triggers</span>';
            }
            html += '<button class="skill-info-btn" title="Show details">i</button>';
            html += '</div>';
            if (tk.auth_warning) {
                html += '<div class="auth-warning-badge">⚠ Not connected — <span style="cursor:pointer;text-decoration:underline;" data-connect-slug="' + escapeHtml(tk.slug) + '">link account</span></div>';
            }
    ...
    function renderEnabledIntegrations() {
        ...
        Object.keys(enabledSkills).forEach(function(k) {
            ...
            var logoHtml = "";
            if (imgSrc) {
                logoHtml = '<div class="skill-icon"><img src="' + escapeHtml(imgSrc) + '" onerror="this.outerHTML=\'<span class=skill-icon>🧩</span>\'" /></div>';
            } else {
                logoHtml = '<div class="skill-icon">🧩</div>';
            }
            var toolCount = 0;
            ...
            if (tk && tk.auth_warning) {
                html += '<div class="auth-warning-badge">⚠ Not connected — <span style="cursor:pointer;text-decoration:underline;" data-connect-slug="' + escapeHtml(slug) + '">link account</span></div>';
            }
            html += '</div>';
        });
        if (count > 0) {
    ...
    async function saveComposioKey() {
        ...
        if (!validated || !toolkitsData) {
            statusEl.innerHTML = '<span style="color:var(--warning);">⚠ Cannot validate from browser (CORS). Key saved for desktop use.</span>';
            return;
        }
