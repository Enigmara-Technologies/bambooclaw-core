# BambooClaw Core Development Environment

A fully containerized development sandbox for BambooClaw Core agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`BambooClaw Core-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `BambooClaw Core` CLI commands manually, debug the binary, or check logs internally.

- **Path**: `/BambooClaw Core-data`
- **User**: `nobody` (65534)
...
- **Agent**: `/BambooClaw Core-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.BambooClaw Core` (mounted to `/BambooClaw Core-data/.BambooClaw Core`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.BambooClaw Core` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `BambooClaw Core-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=BambooClaw Core-target`) to speed repeat local image builds.
