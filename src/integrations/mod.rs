...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `BambooClaw Core onboard --interactive` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: BambooClaw Core onboard");
            println!("    4. Start: BambooClaw Core channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: BambooClaw Core onboard");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "OpenRouter" => {
            ...
            println!("    2. Run: BambooClaw Core onboard");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    BambooClaw Core can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.BambooClaw Core/workspace/cron/");
            println!("    Run: BambooClaw Core cron list");
        }
        "Webhooks" => {
            println!("  Built-in:");
            println!("    HTTP endpoint for external triggers.");
            println!("    Run: BambooClaw Core gateway");
        }
        _ => {
            if status == IntegrationStatus::ComingSoon {
                println!("  This integration is planned. Stay tuned!");
                println!("  Track progress: https://github.com/theonlyhennygod/BambooClaw Core");
            }
        }
    }
