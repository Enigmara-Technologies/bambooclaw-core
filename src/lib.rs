...
Examples:
  ...
  BambooClawCore channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  BambooClawCore channel bind-telegram 123456789")]
    BindTelegram {
...
Examples:
  ...
  BambooClawCore cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  BambooClawCore cron add-every 60000 'Ping heartbeat'     # every minute
  BambooClawCore cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  BambooClawCore cron once 2h 'Follow up on deployment'
  BambooClawCore cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  BambooClawCore cron update <task-id> --expression '0 8 * * *'
  BambooClawCore cron update <task-id> --tz Europe/London --name 'Morning check'
  BambooClawCore cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  BambooClawCore hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  BambooClawCore peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash BambooClawCore firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash BambooClawCore firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  BambooClawCore peripheral flash
  BambooClawCore peripheral flash --port /dev/cu.usbmodem12345
  BambooClawCore peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash BambooClawCore firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
