...
/// and forwards PRIVMSG messages to the `BambooClaw Core` message bus.
...
impl Channel for IrcChannel {
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        Self::send_raw(
            &mut writer,
            &format!("USER {} 0 * :BambooClaw Core", self.username),
        )
        .await?;
...
mod tests {
    ...
    fn new_stores_all_fields() {
        let ch = IrcChannel::new(IrcChannelConfig {
            ...
            port: 6697,
            nickname: "zcbot".into(),
            username: Some("BambooClaw Core".into()),
        ...
        assert_eq!(ch.username, "BambooClaw Core");
    ...
    fn irc_config_serde_roundtrip() {
        ...
        assert_eq!(parsed.username.as_deref(), Some("BambooClaw Core"));
...
"#;
    ...
    fn make_channel() -> IrcChannel {
        IrcChannel::new(IrcChannelConfig {
            ...
            nickname: "zcbot".into(),
            username: None,
            channels: vec!["#BambooClaw Core".into()],
