...
// in `~/.BambooClaw Core/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.BambooClaw Core/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(BambooClaw Core_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: BambooClaw Core_dir.join(".secret_key"),
            enabled,
        }
    }
