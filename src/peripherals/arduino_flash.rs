//! Flash BambooClawCore Arduino firmware via arduino-cli.
//!
...
/// BambooClawCore Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/BambooClawCore-arduino/BambooClawCore-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "BambooClawCore-arduino";
...
/// Flash BambooClawCore firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("BambooClawCore_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling BambooClawCore Arduino firmware...");
    ...
    println!("BambooClawCore firmware flashed successfully.");
