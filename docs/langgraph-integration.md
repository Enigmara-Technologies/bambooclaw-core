...
This guide explains how to use the `BambooClaw Core-tools` Python package for consistent tool calling with any OpenAI-compatible LLM provider.

## Background

Some LLM providers, particularly Chinese models like GLM-5 (Zhipu AI), have inconsistent tool calling behavior when using text-based tool invocation. BambooClaw Core's Rust core uses structured tool calling via the OpenAI API format, but some models respond better to a different approach.
...
```bash
pip install BambooClaw Core-tools
```
...
```python
import asyncio
from BambooClaw Core_tools import create_agent, shell, file_read, file_write
...
```python
from BambooClaw Core_tools import tool, create_agent
...
```python
import os
from BambooClaw Core_tools.integrations import DiscordBot
...
# Single message
BambooClaw Core-tools "What is the current date?"

# Interactive mode
BambooClaw Core-tools -i
```

## Comparison with Rust BambooClaw Core

| Aspect | Rust BambooClaw Core | BambooClaw Core-tools |
|--------|---------------|-----------------|
