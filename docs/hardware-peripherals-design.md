# Hardware Peripherals Design — BambooClaw Core

BambooClaw Core enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** BambooClaw Core acts as a hardware-aware AI agent that:
...
**Mental model:** BambooClaw Core = brain that understands hardware. Peripherals = arms and legs it controls.
...
BambooClaw Core runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  BambooClaw Core on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
2. BambooClaw Core fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
BambooClaw Core runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.

```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  BambooClaw Core on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
2. BambooClaw Core identifies connected hardware (VID/PID, architecture)
...
2. BambooClaw Core writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. BambooClaw Core auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| BambooClaw Core runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs BambooClaw Core; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

BambooClaw Core on Pi; GPIO via rppal or sysfs. No separate firmware.
...
board = "esp32"
transport = "wifi"
# Edge-Native: BambooClaw Core runs on ESP32
```
...
1. **Startup:** BambooClaw Core loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between BambooClaw Core and peripherals:
...
- **BambooClaw Core-firmware** or **BambooClaw Core-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; BambooClaw Core connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`BambooClaw Core peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `BambooClaw Core hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `BambooClaw Core hardware introspect <path>`: memory map, peripheral list
...
- [x] BambooClaw Core on Raspberry Pi (native GPIO via rppal)
...
- [x] `BambooClaw Core-esp32` firmware crate (`firmware/BambooClaw Core-esp32`) — GPIO over UART
- [x] ESP32 in hardware registry (CH340 VID/PID)
- [ ] BambooClaw Core *on* ESP32 (WiFi + LLM, edge-native) — future
- [ ] Wasm or template-based execution for LLM-generated logic

**Usage:** Flash `firmware/BambooClaw Core-esp32` to ESP32, add `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` to config.
...
- Running full BambooClaw Core *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). BambooClaw Core runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. BambooClaw Core gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: BambooClaw Core from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey BambooClaw Core, what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
