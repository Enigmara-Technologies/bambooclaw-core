# BambooClaw Core Security Improvement Roadmap
...
BambooClaw Core already has **excellent application-layer security**:
...
## Comparison: BambooClaw Core vs PicoClaw vs Production Grade

| Feature | PicoClaw | BambooClaw Core Now | BambooClaw Core + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`BambooClaw Core audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/BambooClaw Core/audit.log"
sign_events = true
max_size_mb = 100
...
BambooClaw Core security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
BambooClaw Core audit --user @alice --since 24h
BambooClaw Core audit --risk high --violations-only
BambooClaw Core audit --verify-signatures

# Sandbox test
BambooClaw Core sandbox --test
# → Testing isolation...
...
**BambooClaw Core is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, BambooClaw Core becomes:
...
**Value**: Transforms BambooClaw Core from "safe for testing" to "safe for production"
