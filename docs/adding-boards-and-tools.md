# Adding Boards and Tools — BambooClaw Core Hardware Guide

This guide explains how to add new hardware boards and custom tools to BambooClaw Core.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.BambooClaw Core/config.toml)
BambooClaw Core peripheral add nucleo-f401re /dev/ttyACM0
BambooClaw Core peripheral add arduino-uno /dev/cu.usbmodem12345
BambooClaw Core peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, BambooClaw Core can index PDF files:
...
2. **Add to config** — `BambooClaw Core peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `BambooClaw Core peripheral list` | List configured boards |
| `BambooClaw Core peripheral add <board> <path>` | Add board (writes config) |
| `BambooClaw Core peripheral flash` | Flash Arduino firmware |
| `BambooClaw Core peripheral flash-nucleo` | Flash Nucleo firmware |
| `BambooClaw Core hardware discover` | List USB devices |
| `BambooClaw Core hardware info` | Chip info via probe-rs |
