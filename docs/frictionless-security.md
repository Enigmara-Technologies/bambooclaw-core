...
[INFO] ✓ Audit logging enabled (~/.config/BambooClaw Core/audit.log)
...
```toml
# ~/.config/BambooClaw Core/config.toml
...
```bash
# Check what's active
$ BambooClaw Core security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/BambooClaw Core/audit.log
  → 47 events logged today

# Disable sandbox explicitly (writes to config)
$ BambooClaw Core config set security.sandbox.enabled false

# Enable specific backend
$ BambooClaw Core config set security.sandbox.backend firejail

# Adjust limits
$ BambooClaw Core config set security.resources.max_memory_mb 2048
```
...
```bash
$ BambooClaw Core onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Status visibility** — `BambooClaw Core security --status`
