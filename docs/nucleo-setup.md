# BambooClaw Core on Nucleo-F401RE — Step-by-Step Guide

Run BambooClaw Core on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

BambooClaw Core can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
BambooClaw Core includes everything for Nucleo-F401RE:

| Component | Location | Purpose |
|-----------|----------|---------|
| Firmware | `firmware/BambooClaw Core-nucleo/` | Embassy Rust — USART2 (115200), gpio_read, gpio_write |
| Serial peripheral | `src/peripherals/serial.rs` | JSON-over-serial protocol (same as Arduino/ESP32) |
| Flash command | `BambooClaw Core peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via BambooClaw Core

From the BambooClaw Core repo root:

```bash
BambooClaw Core peripheral flash-nucleo
```

This builds `firmware/BambooClaw Core-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.

### 1.3 Manual Flash (Alternative)

```bash
cd firmware/BambooClaw Core-nucleo
cargo build --release --target thumbv7em-none-eabihf
probe-rs run --chip STM32F401RETx target/thumbv7em-none-eabihf/release/BambooClaw Core-nucleo
```
...
Add to `~/.BambooClaw Core/config.toml`:
...
```bash
BambooClaw Core agent --message "Turn on the LED on pin 13"
```
...
| 3 | `BambooClaw Core peripheral flash-nucleo` |
| 4 | Add Nucleo to config.toml (path = your serial port) |
| 5 | `BambooClaw Core daemon` or `BambooClaw Core agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `BambooClaw Core peripheral list` to verify.
