# BambooClaw Core Troubleshooting
...
- `cargo check` / `cargo build` appears stuck at `Checking BambooClaw Core` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in BambooClaw Core:
...
### `BambooClaw Core` command not found after install

Symptom:

- install succeeds but shell cannot find `BambooClaw Core`

Fix:

```bash
export PATH="$HOME/.cargo/bin:$PATH"
which BambooClaw Core
```
...
Verify `~/.BambooClaw Core/config.toml`:
...
- stop extra `BambooClaw Core daemon` / `BambooClaw Core channel start` processes
