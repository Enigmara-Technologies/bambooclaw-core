...
Complete guide to setting up a BambooClaw Core-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.BambooClaw Core/models
cp models/ggml-base.bin ~/.BambooClaw Core/models/
```
...
mkdir -p ~/.BambooClaw Core/models/piper
cd ~/.BambooClaw Core/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.BambooClaw Core/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build BambooClaw Core Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/theonlyhennygod/BambooClaw Core
cd BambooClaw Core

# Build robot kit
cargo build --release -p BambooClaw Core-robot-kit

# Build main BambooClaw Core (optional, if using as agent)
cargo build --release
```
...
```toml
# ~/.BambooClaw Core/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.BambooClaw Core/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/BambooClaw Core_sensors.fifo"
...
### Start BambooClaw Core Agent

```bash
# Configure BambooClaw Core to use robot tools
cat > ~/.BambooClaw Core/config.toml << 'EOF'
...
cp ~/BambooClaw Core/crates/robot-kit/SOUL.md ~/.BambooClaw Core/workspace/

# Start agent
./target/release/BambooClaw Core agent
```
...
# Start sensor loop
if [ ! -p /tmp/BambooClaw Core_sensors.fifo ]; then
    mkfifo /tmp/BambooClaw Core_sensors.fifo
fi
python3 ~/sensor_loop.py &
SENSOR_PID=$!

# Start BambooClaw Core
cd ~/BambooClaw Core
./target/release/BambooClaw Core daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/BambooClaw Core-robot.service
sudo tee /etc/systemd/system/BambooClaw Core-robot.service << 'EOF'
[Unit]
Description=BambooClaw Core Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/BambooClaw Core
...
sudo systemctl enable BambooClaw Core-robot
sudo systemctl start BambooClaw Core-robot

# Check status
sudo systemctl status BambooClaw Core-robot
journalctl -u BambooClaw Core-robot -f  # View logs
```
