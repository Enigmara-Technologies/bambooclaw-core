...
This guide covers testing the Telegram channel integration for BambooClaw Core.
...
6. **Health check timeout**
    ...
    ```bash
    time BambooClaw Core channel doctor
    ```
...
Solution: Check user allowlist
  ...
  4. Run: BambooClaw Core onboard --channels-only
```
...
# 3. Monitor logs
RUST_LOG=info BambooClaw Core daemon
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time BambooClaw Core channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep BambooClaw Core`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/BambooClaw Core` |
...
```bash
# Maximum logging
RUST_LOG=trace BambooClaw Core channel start
...
# Validate config
cat ~/.BambooClaw Core/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
# 4. Restart service
BambooClaw Core service restart
...
- [BambooClaw Core Main README](README.md)
- [Contributing Guide](CONTRIBUTING.md)
- [Issue Tracker](https://github.com/theonlyhennygod/BambooClaw Core/issues)
